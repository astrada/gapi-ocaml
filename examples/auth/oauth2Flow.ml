...
let get_access_token code (cgi : Netcgi.cgi_activation) =
  ...
  Common.do_request (fun session ->
      ...
      let output =
        Printf.sprintf
          "<form method=\"post\" action=\"\"><fieldset><legend>OAuth2 \
           tokens:</legend>oa2_token: <input type=\"text\" name=\"oa2_token\" \
           value=\"%s\" size=\"50\" /><br />oa2_refresh: <input type=\"text\" \
           name=\"oa2_refresh\" value=\"%s\" size=\"50\" /><br /><input \
           type=\"submit\" name=\"save\" value=\"Save\" \
           /></fieldset></form><p>token_type: %s</p><p>expires_in: %d</p>"
