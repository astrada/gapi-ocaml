...
struct
  ...
  let rec parse x tree =
    match tree with
      ...
      | e ->
          unexpected ("GapiCalendar.Person.parse") e
  ...
  let rec parse x tree =
    match tree with
      ...
      | AnnotatedTree.Leaf
          ...
          { x with timeZone = v }
      | e ->
          unexpected ("GapiCalendar.DateTime.parse") e
    ...
    match tree with
      ...
      | AnnotatedTree.Node
          ...
          parse_children
            ...
            cs
      | e ->
          unexpected ("GapiCalendar.Attendee.parse") e
  ...
  let parse x tree =
    let parse_property _ = function
        AnnotatedTree.Leaf
          ...
          (n, v)
      | e ->
          unexpected "GapiCalendar.ExtendedProperties.parse_property" e
    in
      match tree with
          AnnotatedTree.Node
        ...
        | AnnotatedTree.Node
            ...
            parse_collection
              ...
              cs
        | e ->
            unexpected "GapiCalendar.ExtendedProperties.parse" e
  ...
  let rec parse x tree =
    let parse_preference _ = function
        AnnotatedTree.Leaf
          ...
          (n, v)
      | e ->
          unexpected "GapiCalendar.Gadget.parse_preference" e
    in
      match tree with
          AnnotatedTree.Leaf
            ({ name = "type"; data_type = Scalar },
...
struct
  ...
  let empty = {
    ...
    location = "";
    colorId = "";
    creator = Person.empty;
    organizer = Person.empty;
    start = DateTime.empty;
    _end = DateTime.empty;
    recurrence = [];
    recurringEventId = "";
    originalStartTime = DateTime.empty;
    transparency = "";
    visibility = "";
    iCalUID = "";
    sequence = 0;
  ...
  let render x =
    render_object ""
      [render_string_value "kind" x.kind;
       ...
       render_string_value "location" x.location;
       render_string_value "colorId" x.colorId;
       render_object "creator" (Person.render x.creator);
       render_object "organizer" (Person.render x.organizer);
       render_object "start" (DateTime.render x.start);
       render_object "end" (DateTime.render x._end);
       render_array "recurrence" (render_string_value "") x.recurrence;
       render_string_value "recurringEventId" x.recurringEventId;
       render_object "originalStartTime" (DateTime.render x.originalStartTime);
       ...
       render_array "attendees" Attendee.render x.attendees;
       render_bool_value "attendeesOmitted" x.attendeesOmitted;
       ExtendedProperties.render x.extendedProperties;
       Gadget.render x.gadget;
  ...
  let parse_recurrence _ = function
      AnnotatedTree.Leaf
        ...
        v
    | e ->
        unexpected "GapiCalendar.EventsResource.parse_recurrence" e
    ...
    match tree with
      ...
      | AnnotatedTree.Node
          ({ name = "items"; data_type = Array },
           cs) ->
          parse_collection
            EventsResource.parse
            EventsResource.empty
            (fun xs -> { x with items = xs })
            cs
      | AnnotatedTree.Node
          ({ name = ""; data_type = Object },
          ...
          parse_children
            ...
            cs
      | e ->
          unexpected "GapiCalendar.EventsList.parse" e
