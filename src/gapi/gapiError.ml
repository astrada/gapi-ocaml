...
module SingleError = struct
  ...
  let rec parse x tree =
    match tree with
    | AnnotatedTree.Leaf ({ name = "domain"; data_type = Scalar }, `String v) ->
        { x with domain = v }
    | AnnotatedTree.Leaf ({ name = "reason"; data_type = Scalar }, `String v) ->
        { x with reason = v }
    | AnnotatedTree.Leaf ({ name = "message"; data_type = Scalar }, `String v)
      ->
        { x with message = v }
    | AnnotatedTree.Leaf ({ name = "message"; data_type = Scalar }, `Null) -> x
    | AnnotatedTree.Leaf
        ({ name = "locationType"; data_type = Scalar }, `String v) ->
        { x with locationType = v }
    | AnnotatedTree.Leaf ({ name = "location"; data_type = Scalar }, `String v)
      ->
        { x with location = v }
    | AnnotatedTree.Leaf
        ({ name = "extendedHelp"; data_type = Scalar }, `String v) ->
        { x with extendedHelp = v }
    | AnnotatedTree.Node ({ name = ""; data_type = Object }, cs) ->
        parse_children parse empty Fun.id cs
    | e -> unexpected "GapiService.SingleError.parse" e x
end

module RequestError = struct
  type t = { errors : SingleError.t list; code : int; message : string }
  ...
  let empty = { errors = []; code = 0; message = "" }
  ...
  let rec parse x tree =
    match tree with
    | AnnotatedTree.Node ({ name = "errors"; data_type = Array }, cs) ->
        parse_collection SingleError.parse SingleError.empty
          (fun xs -> { x with errors = xs })
          cs
    | AnnotatedTree.Leaf ({ name = "code"; data_type = Scalar }, `Int v) ->
        { x with code = v }
    | AnnotatedTree.Leaf ({ name = "message"; data_type = Scalar }, `String v)
      ->
        { x with message = v }
    | AnnotatedTree.Node ({ name = "error"; data_type = Object }, cs) ->
        parse_children parse empty Fun.id cs
    | AnnotatedTree.Node ({ name = ""; data_type = Object }, cs) ->
        parse_children parse empty Fun.id cs
